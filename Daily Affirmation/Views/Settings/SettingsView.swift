            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        NavigationLink(destination: PinnedQuotesDetailView(quoteManager: quoteManager)) {
                            SettingsCard(
                                icon: "pin.fill",
                                title: "Pinned Quote",
                                subtitle: SharedQuoteManager.shared.isPinned() ? "Tap to manage" : "No quote pinned",
                                iconColor: Color(red: 0.659, green: 0.902, blue: 0.812)
                            )
                        }
                        .buttonStyle(PlainButtonStyle())
                        .accessibilityIdentifier("pinned_quotes_section")
                        .accessibility(addTraits: .isButton)
...
struct PinnedQuotesDetailView: View {
    @ObservedObject var quoteManager: QuoteManager
    @Environment(\.dismiss) private var dismiss
    @State private var showingUnpinConfirmation = false
    
    private var pinnedQuote: String? {
        SharedQuoteManager.shared.getPinnedQuote()
    }
    
    private var hasPinnedQuote: Bool {
        SharedQuoteManager.shared.isPinned() && pinnedQuote != nil
    }
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                        ...
                        .fill(
                            LinearGradient(
                                colors: [
                                    ...
                                    Color(red: 0.659, green: 0.902, blue: 0.812).opacity(0.05)
                                ],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            )
                ...
                if !hasPinnedQuote {
                    VStack(spacing: 20) {
                        ...
                        Text("No pinned quote")
                            .font(.title2)
                            .fontWeight(.semibold)
                            .foregroundColor(.black)
                        
                        Text("Pin a quote from the main screen to keep it displayed in your widgets permanently!")
                            .font(.body)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                            .padding(.horizontal, 40)
                    ...
                    VStack(spacing: 24) {
                        ...
                        .padding(.horizontal, 24)
                        .accessibilityLabel("Unpin current quote")
                        
                        // Info Text
                        Text("When you unpin this quote, your widgets will return to showing daily quotes. You can pin a new quote anytime from the main screen.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                            .padding(.horizontal, 40)
        ...
        .navigationBarHidden(true)
        .alert("Unpin Quote", isPresented: $showingUnpinConfirmation) {
            ...
            Text("Are you sure you want to unpin this quote? Your widgets will return to showing daily quotes.")
        }
    }
}
